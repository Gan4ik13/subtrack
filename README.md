...
Простой сайт для отслеживания подписок. Бесплатно до 3 подписок, Premium (безлимит + Telegram-уведомления о списаниях) — 99 ₽/мес, 499 ₽/год или 799 ₽ навсегда.
...
- `backend/` — API на FastAPI. Размещается на Render.
  ...
  - Платежи: ЮKassa, ЮMoney, Crypto Pay или ручной режим
...
| `PAYMENT_MODE` | `manual` | `manual` \| `yookassa` \| `yoomoney` \| `cryptopay` |
| `PRICE_RUB` | `99` | Цена Premium за месяц |
| `YOOKASSA_SHOP_ID` | — | Идентификатор магазина ЮKassa |
| `YOOKASSA_SECRET_KEY` | — | Секретный ключ ЮKassa |
| `YOOKASSA_VAT_CODE` | `1` | Код НДС для чека ЮKassa |
...
2. Пользователь нажимает «Подключить через бота» в настройках и запускает deep-link бота. Числовой ID остаётся запасным способом подключения.
3. Планировщик проверяет подписки раз в час, переносит прошедшие даты на следующий период и шлёт напоминание за 3 дня до списания (только Premium).
